...
async function convexCall(kind, path, args) {
  const url = convexUrl();
  if (!url) {
    const error = new Error('Convex is not configured. Set CONVEX_URL in Netlify.');
    error.statusCode = 503;
    throw error;
  }

  const headers = { 'Content-Type': 'application/json' };
  if (process.env.CONVEX_DEPLOY_KEY) headers.Authorization = `Convex ${process.env.CONVEX_DEPLOY_KEY}`;
